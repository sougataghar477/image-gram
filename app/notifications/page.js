...
{notifications?.length>0 ? [...(notifications || [])].reverse()?.map((notification,index)=>{
    if(notification.notificationType==="like" && userContext.state.user.username !== notification.notifier){
        return <p onClick={() =>{console.log(notification);
            ...
            <Link href={'/profile/'+notification.notifier}><b>{notification.notifier}</b></Link> liked your <Link className="underline" href={'https://image-gram-neon.vercel.app/profile/'+userContext?.state?.user?.username+'/'+notification.postId}>post </Link>
{dayjs(notification?.createdAt).fromNow()}
    ...
    return <p onClick={() =>{console.log(dayjs(notification.createdAt).fromNow());setTimeout(() => {
        ...
        <Link href={'/profile/'+notification?.notifier}><b>{notification?.notifier}</b></Link> replied to your comment {notification.reply} on your <Link className="underline" href={'https://image-gram-neon.vercel.app/profile/'+userContext?.state?.user?.username+'/'+notification.postId}>post </Link>{''} {dayjs(notification.createdAt).fromNow()}
    </p>
    }
    if(notification.notificationType==='liked comment' && userContext.state.user.username !== notification.notifier){
        return <p onClick={() =>{console.log(dayjs(notification.createdAt).fromNow());
            ...
            <Link href={'/profile/'+notification?.notifier}><b>{notification?.notifier}</b></Link> liked your comment `{notification.comment}` on your <Link className="underline" href={'https://image-gram-neon.vercel.app/profile/'+notification.postAuthor+'/'+notification.postId}>post </Link>
            {' '} {dayjs(notification?.createdAt).fromNow()}
        </p>
        }
        // Liked Reply
